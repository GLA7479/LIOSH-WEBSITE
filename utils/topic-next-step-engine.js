...
function buildHebrewCopy(step, ctx, cfg) {
  ...
  const table = {
    ...
    maintain_and_strengthen: {
      ...
      parentHe: `בנושא ${displayName} מומלץ להמשיך כרגע באותה רמת קושי, להוסיף תרגול קצר וממוקד פעמיים בשבוע. המטרה: ביסוס ועקביות לפני שמשנים משהו.`,
    ...
    remediate_same_level: {
      ...
      parentHe:
        "מומלץ להישאר על אותה רמת קושי ולהתמקד בהבנת הטעויות: לאחר כל תשובה שגויה — נבדוק בדיוק איפה טעינו, וההמלצה שלנו לא לעלות רמה עד שנרגיש שיש התקדמות ועקביות.",
    ...
    return applyAggressiveEvidenceCap(
      {
        ...
        parentHe: `ב«${displayName}» יש עדיין מעט נתונים (${q} שאלות). המלצה להמשיך באותה רמת קושי, להוסיף שניים־שלושה מפגשי תרגול קצרים כדי שההמלצה הבאה תהיה מדויקת יותר.`,
