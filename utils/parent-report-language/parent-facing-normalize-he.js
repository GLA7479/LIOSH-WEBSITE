...
export function normalizeParentFacingHe(raw) {
  ...
  const phrasePairs = [
    [/שליטה\s+יציבה\s+בנשיאה/giu, "שליטה טובה ועקבית בהעברה בין עמודות (חיבור)"],
    [/מאסטרי\s+יציב/giu, "שליטה טובה ויציבה"],
    [/מאסטרי\b/giu, "שליטה טובה"],
    [/פרופיל\s+מאסטרי/giu, "פרופיל של שליטה טובה"],
    [/אלגוריתם\s+של\s+חיבור\s+עם\s+העברה/giu, "דרך עבודה מסודרת בחיבור עם העברה"],
    [/דגלי\s+סיכון\s+פעילים/giu, "נקודות לשימת לב"],
    [/מזהה\s+טקסונומיה\s+[MH]-\d{2,}/giu, ""],
    [/לפי\s+טקסונומיה\s+[MH]-\d{2,}/giu, ""],
    [/כלל\s+בסיס\s+במנוע/giu, "כלל פנימי של המערכת"],
  ];
  ...
  // כל מופע נותר של «נשיאה» (אחרי כללי הפדגוגיה) — במסלול דוח הורים זה כמעט תמיד העברה עשרונית
  if (/נשיאה/u.test(s)) {
