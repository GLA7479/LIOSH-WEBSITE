...
const INTENT_PARAPHRASES = {
  ...
  off_topic_redirect: [
    ...
    /מה\s*השעה|כמה\s*השעה|שעון/u,
    /תספר\s*בדיחה|בדיחה/u,
    /מי\s*ראש\s*הממשלה|ראש\s*הממשלה/u,
    /מתכון|עוגה/u,
...
function offTopicUtteranceHeuristic(t, folded) {
  ...
  return (
    ...
    /מה\s*השעה|כמה\s*השעה/u.test(s) ||
    /תספר\s*בדיחה|בדיחה/u.test(s) ||
    /מי\s*ראש\s*הממשלה/u.test(s) ||
    /מתכון|עוגה/u.test(s) ||
