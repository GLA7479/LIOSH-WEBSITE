...
function expectedPatternForQuestion(q) {
  if (q === "מה המקצוע החזק?")
    return /המקצוע החזק ביותר הוא|מקצוע אחד עם תרגול|לא .*מספיק תרגול.*שני מקצועות|לא נדרגים כאן מקצועות/;
  if (q === "מה המקצוע החלש?")
    return /המקצוע הנמוך ביותר כרגע הוא|מקצוע אחד עם תרגול|לא .*מספיק תרגול.*שני מקצועות|לא נדרגים כאן מקצועות/;
  if (q === "באיזה מקצוע הכי קשה?")
    return /המקצוע שבו הכי «קשה»|מקצוע אחד עם תרגול|לא .*מספיק תרגול.*שני מקצועות|לא נדרגים כאן מקצועות/;
