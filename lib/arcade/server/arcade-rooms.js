...
export async function createArcadeRoom(supabase, params) {
  ...
  const maxPlayersUnspecified =
    maxPlayersArg == null || maxPlayersArg === "" || (typeof maxPlayersArg === "string" && !String(maxPlayersArg).trim());

  let maxPlayers;
  if (gameKey === "ludo" && maxPlayersUnspecified) {
    maxPlayers = 2;
  } else {
    maxPlayers = Number(maxPlayersArg || game.max_players);
...
export async function joinArcadeRoomById(supabase, studentId, roomId) {
  ...
  if (joinedCount >= room.max_players) {
    if (room.game_key === "fourline") {
      await maybeStartFourlineSession(supabase, roomId);
    } else if (room.game_key === "ludo") {
      const start = await maybeStartLudoSession(supabase, roomId);
      if (start.error) {
        await supabase
          .from("arcade_room_players")
          .update({ left_at: new Date().toISOString() })
          .eq("id", ins.data.id);
        await refundArcadeEntry(supabase, studentId, room.entry_cost, `arcade:refund:ludo_start_failed:${roomId}:${studentId}`, {
          sourceId: roomId,
        });
        return {
          error: {
            code: start.error.code || "session_start_failed",
            message: start.error.message || "לא ניתן להתחיל את משחק הלודו",
          },
        };
      }
      const sessionOk = start.ok === true && start.session != null;
      const alreadyHadSession = start.skipped === true && start.already === true;
      if (!sessionOk && !alreadyHadSession) {
        await supabase
          .from("arcade_room_players")
          .update({ left_at: new Date().toISOString() })
          .eq("id", ins.data.id);
        await refundArcadeEntry(supabase, studentId, room.entry_cost, `arcade:refund:ludo_start_skipped:${roomId}:${studentId}`, {
          sourceId: roomId,
        });
        return {
          error: {
            code: "session_start_failed",
            message: "לא ניתן להפעיל את משחק הלודו — נסה שוב",
          },
        };
      }
    }
