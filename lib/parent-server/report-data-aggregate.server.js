...
const ALLOWED_MODE_VALUES = Object.freeze(["learning", "review", "drill"]);
...
function readModeLevelFromObject(obj) {
  ...
  return {
    mode: pickEnumString(obj.mode, ALLOWED_MODE_VALUES),
...
export async function aggregateParentReportPayload(serviceClient, student, fromDate, toDate) {
  ...
  for (const answer of answers) {
    ...
    const resolvedMode =
      payloadModeLevel.mode !== "unknown"
        ? payloadModeLevel.mode
        : clientModeLevel.mode !== "unknown"
        ? clientModeLevel.mode
        : sessionRef?.mode || "unknown";
    const resolvedLevel =
