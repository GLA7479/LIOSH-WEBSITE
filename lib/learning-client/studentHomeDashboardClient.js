 ...
 * Data must come from GET /api/student/me and GET /api/student/learning-profile only.
 */

import { LEARNING_PROFILE_SUBJECT_KEYS } from "../learning-shared/student-learning-profile-model.js";
import {
  buildAccountSnapshotForParentReport,
  mapSubjectAccountView,
} from "../learning-shared/student-account-state-view.js";
 ...
 * @param {{ row: object, derived: object } | null | undefined} learningProfile from /api/student/learning-profile
 */
export function buildStudentHomeView({ student, learningProfile }) {
  const row =
    learningProfile?.row && typeof learningProfile.row === "object" && !Array.isArray(learningProfile.row)
      ? learningProfile.row
      : {};
  const derived =
    learningProfile?.derived && typeof learningProfile.derived === "object" && !Array.isArray(learningProfile.derived)
      ? learningProfile.derived
      : {};
  ...
  const snap = buildAccountSnapshotForParentReport(row, derived, name);

  let correctAll = 0;
  let wrongAll = 0;
  for (const sk of LEARNING_PROFILE_SUBJECT_KEYS) {
    const b = derived.bySubject?.[sk];
    if (!b) continue;
  ...
  for (const sk of LEARNING_PROFILE_SUBJECT_KEYS) {
    ...
    sessionMinutesAll += n(derived.bySubject?.[sk]?.sessionMinutesTotal);
  }
  ...
  for (const sk of STUDENT_HOME_SUBJECT_ORDER) {
    ...
    if (!meta) continue;
    const subSnap = snap.bySubject?.[sk];
    const dSub = derived.bySubject?.[sk];
  ...
  let topAnswers = -1;
  for (const sk of STUDENT_HOME_SUBJECT_ORDER) {
    const at = n(derived.bySubject?.[sk]?.answersTotal);
    if (at > topAnswers) {
      topAnswers = at;
      topKey = sk;
    }
  ...
  return {
    identity: {
      ...
      gradeLevel: student?.grade_level != null && student?.grade_level !== "" ? String(student.grade_level) : "",
    ...
    recommendations,
    meta: {
      hasLearningProfile: !!(learningProfile?.row && learningProfile?.derived),
